"""
Evaluates the native SQuAD 2.0 no-answer baseline and reports overall, answerable-only, and unanswerable-only selective QA metrics.
"""
...
DEFAULT_EVALUATED_OUTPUT_PATH = Path(
    "outputs/predictions/native_no_answer_baseline_evaluated_test.jsonl"
)
...
def build_baseline_summary(
    ...
    answerable = [
        ...
        if bool(prediction["is_answerable"])
    ]
    unanswerable = [
        prediction for prediction in predictions
        if not bool(prediction["is_answerable"])
    ]

    first_prediction = predictions[0]

    return {
        "evaluation_type": "native_no_answer_baseline",
        "system": first_prediction.get("system", "unknown"),
        "model": first_prediction.get("model", "unknown"),
        "split": first_prediction.get("split", "unknown"),
        "total_predictions": len(predictions),
        "metrics": calculate_metrics(predictions),
        "answerable_only": calculate_metrics(answerable),
        "unanswerable_only": calculate_metrics(unanswerable),
    }
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate the native SQuAD2 no-answer baseline "
